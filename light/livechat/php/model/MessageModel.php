    ...
    public function preSave()
    {
        ...
        if(!isset($this->id)) // New message
        {
            ...
            if($this->to_id != self::TALK_ID_CANCELLED && !($fromUser->hasRole('OPERATOR') && $toUser->hasRole('OPERATOR')))
            {
                ...
                if($this->to_id == self::TALK_ID_ALL && $lastMessage && $lastMessage->to_id != self::TALK_ID_ALL) // it's sent as a "broadcast" message, but has a talk already
                {
        ...
        if(isset($query['name']) || isset($query['mail']))
        {
            foreach($groups as $group)
            {
                ...
                foreach($group as $message)
                {
                    ...
                    {
                        if(
                            (is_array($fromUser) && stripos($fromUser['name'], $query['name']) !== false) ||
                            (is_array($toUser)   && stripos($toUser['name'],   $query['name']) !== false)
                        )
                        {
                            $addEntry = true;
                        }
                    ...
                    {
                        if(
                            (is_array($fromUser) && stripos($fromUser['mail'], $query['mail']) !== false) ||
                            (is_array($toUser)   && stripos($toUser['mail'],   $query['mail']) !== false)
                        )
                        {
                            $addEntry = true;
                        }
